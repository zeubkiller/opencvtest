...
class HoughLineProcessing:
    ...
    def __init__(self, app, img):
        ...
        self._theta = ValueUpdater('Theta', np.pi/180, 0.0, 2 * np.pi, self.apply)
    ...
    def apply(self):
        """Apply the method"""
        lines = cv2.HoughLinesP(self._canny.processed_img, self._rho.value, self._theta.value, self._threshold.value, minLineLength=self._min_line_length.value, maxLineGap=self._max_line_gap.value)
